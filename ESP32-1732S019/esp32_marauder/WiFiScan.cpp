...
void WiFiScan::activeEapolSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type)
{
  ...
  void WiFiScan::eapolMonitorMain(uint32_t currentTime)
  {
    for (x_pos = (11 + x_scale); x_pos <= 320; x_pos = x_pos)
    {
      ...
      // Button handling adjusted for 170x320 landscape
      if (b == 4 || b == 5 || b == 6) {
        delay(70);
        display_obj.tft.fillRect(110, 0, 210, 25, TFT_BLACK);
        display_obj.tftDrawChannelScaleButtons(set_channel);
        display_obj.tftDrawExitScaleButtons();

        if (b == 4 && set_channel > 1) {
          set_channel--;
          changeChannel();
        }
        else if (b == 5 && set_channel < MAX_CHANNEL) {
          set_channel++;
          changeChannel();
        }
        else if (b == 6) {
      ...
      if (currentTime - initTime >= (GRAPH_REFRESH * 5)) {
        ...
        y_pos_x = ((-num_eapol * (y_scale * 2)) + (HEIGHT_1 - 2));
        if (y_pos_x >= HEIGHT_1) {
          num_eapol = 0;
        }
        ...
        // Adjusted rectangle erase areas for smaller screen
        if ((x_pos <= 85) || ((x_pos >= 105) && (x_pos <= 320)))
          display_obj.tft.fillRect(x_pos + 1, 25, 8, 70, TFT_BLACK);
        else
          display_obj.tft.fillRect(x_pos + 1, 0, 8, 95, TFT_BLACK);

        display_obj.tft.fillRect(x_pos + 1, 96, 8, 74, TFT_BLACK);

        if (y_pos_x == 95)
          display_obj.tft.drawFastHLine(10, 95, 310, TFT_WHITE);
    ...
    display_obj.tft.fillRect(110, 0, 210, 25, TFT_BLACK);
    display_obj.tft.fillRect(10, 0, 85, 25, TFT_BLACK);
    display_obj.tftDrawChannelScaleButtons(set_channel);
    display_obj.tftDrawExitScaleButtons();
    display_obj.tftDrawEapolColorKey();
  ...
  void WiFiScan::packetMonitorMain(uint32_t currentTime)
  {
    for (x_pos = (11 + x_scale); x_pos <= 320; x_pos = x_pos)
    {
      ...
      // Button handling with adjusted positions
      if (b >= 0 && b <= 6) {
        delay(70);
        display_obj.tft.fillRect(110, 0, 210, 25, TFT_BLACK);
        display_obj.tftDrawXScaleButtons(x_scale);
        ...
        if (b == 0 && x_scale > 1) x_scale--;
        else if (b == 1 && x_scale < 6) x_scale++;
        else if (b == 2 && y_scale > 1) y_scale--;
        else if (b == 3 && y_scale < 9) y_scale++;
        else if (b == 4 && set_channel > 1) {
          set_channel--;
          changeChannel();
        }
        else if (b == 5 && set_channel < MAX_CHANNEL) {
      ...
      if (currentTime - initTime >= GRAPH_REFRESH) {
        ...
        y_pos_x = ((-num_beacon * (y_scale * 2)) + (HEIGHT_1 - 2));
        y_pos_y = ((-num_deauth * (y_scale * 2)) + (HEIGHT_1 - 2));
        y_pos_z = ((-num_probe * (y_scale * 2)) + (HEIGHT_1 - 2));

        num_beacon = num_probe = num_deauth = 0;
        ...
        if ((y_pos_x == 95) || (y_pos_y == 95) || (y_pos_z == 95))
          display_obj.tft.drawFastHLine(10, 95, 310, TFT_WHITE);
...
void WiFiScan::channelAnalyzerLoop(uint32_t tick) {
    ...
    if (tick - this->initTime >= BANNER_TIME) {
      ...
      this->addAnalyzerValue(this->_analyzer_value * BASE_MULTIPLIER, -72, this->_analyzer_values, 320);
      this->_analyzer_value = 0;
...
void WiFiScan::displayAnalyzerString(String str) {
  #ifdef HAS_SCREEN
    display_obj.tft.fillRect(0, 32, 320, 10, TFT_BLACK);
    display_obj.tft.setCursor(0, 32);
    display_obj.tft.setTextSize(1);
    display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
  ...
  (currentScanMode == WIFI_SCAN_ALL))
  {
    ...
    this->packetRateLoop(currentTime);
      ...
      if (currentTime - initTime >= 1000) {
        ...
        #ifdef HAS_SCREEN
          display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
          display_obj.showCenterText(displayString2, 85);
          display_obj.showCenterText(displayString, 85);
        #endif
      }
