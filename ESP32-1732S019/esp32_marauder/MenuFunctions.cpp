...
void MenuFunctions::updateStatusBar()
{
  ...
  if ((wifi_scan_obj.set_channel != wifi_scan_obj.old_channel) || (status_changed)) {
    ...
    #if defined(MARAUDER_MINI) || defined(MARAUDER_M5STICKC) || defined(MARAUDER_REV_FEATHER)
      display_obj.tft.fillRect(80, 0, TFT_WIDTH * 0.21, STATUS_BAR_WIDTH, STATUSBAR_COLOR);
    #else
    ...
    #ifdef HAS_MINI_SCREEN
      display_obj.tft.drawString("CH: " + (String)wifi_scan_obj.set_channel, 80, 0, 1);
    #endif
  }
  ...
  if ((wifi_scan_obj.free_ram != wifi_scan_obj.old_free_ram) || (status_changed)) {
    ...
    display_obj.tft.fillRect(145, 0, 60, STATUS_BAR_WIDTH, STATUSBAR_COLOR);
    #ifdef HAS_FULL_SCREEN
      display_obj.tft.drawString((String)wifi_scan_obj.free_ram + "B", 100, 0, 2);
    #endif

    #ifdef HAS_MINI_SCREEN
      display_obj.tft.drawString((String)wifi_scan_obj.free_ram + "B", 145, 0, 1);
    #endif
  }
  ...
  #ifdef HAS_MINI_SCREEN
    display_obj.tft.setTextColor(the_color, STATUSBAR_COLOR);
    display_obj.tft.drawString("SD", 225, 0, 1);
  #endif
}
...
void MenuFunctions::drawStatusBar()
{
  ...
  wifi_scan_obj.old_channel = wifi_scan_obj.set_channel;
  #ifdef HAS_MINI_SCREEN
    display_obj.tft.fillRect(80, 0, 100, STATUS_BAR_WIDTH, STATUSBAR_COLOR);
  #else
    display_obj.tft.fillRect(50, 0, TFT_WIDTH * 0.21, STATUS_BAR_WIDTH, STATUSBAR_COLOR);
  #endif
  #ifdef HAS_FULL_SCREEN
...
float MenuFunctions::calculateGraphScale(int16_t value) {
  if ((value * this->_graph_scale < 48) && (value * this->_graph_scale > 36)) {
    return this->_graph_scale;
  }
...
float MenuFunctions::graphScaleCheck(const int16_t array[320]) {
  int16_t maxValue = 0;
...
void MenuFunctions::drawMaxLine(int16_t value, uint16_t color) {
  display_obj.tft.drawLine(0, 169 - (value * this->_graph_scale), 320, 169 - (value * this->_graph_scale), color);
  display_obj.tft.setCursor(0, 169 - (value * this->_graph_scale));
...
void MenuFunctions::drawGraph(int16_t *values) {
  ...
  for (int i = 319; i >= 0; i--) {
    if (values[i] >= 0) {
      ...
      display_obj.tft.drawLine(i, 169, i, 100, TFT_BLACK);
      display_obj.tft.drawLine(i, 169, i, 169 - (values[i] * this->_graph_scale), TFT_CYAN);
    }
    else {
      int16_t ch_val = values[i] * -1;
      display_obj.tft.drawLine(i, 169, i, 100, TFT_BLACK);
      display_obj.tft.drawLine(i, 169, i, 100, TFT_RED);
      display_obj.tft.setCursor(i, 100);
  ...
  this->drawMaxLine(maxValue, TFT_GREEN);
  this->drawMaxLine(total / 320, TFT_ORANGE);
}

void MenuFunctions::renderGraphUI(uint8_t scan_mode) {
  display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
  if (scan_mode == WIFI_SCAN_CHAN_ANALYZER)
    display_obj.tft.drawCentreString("Frames/" + (String)BANNER_TIME + "ms", 160, 45, 1);
  else if (scan_mode == BT_SCAN_ANALYZER)
    display_obj.tft.drawCentreString("BLE Beacons/" + (String)BANNER_TIME + "ms", 160, 45, 1);

  display_obj.tft.drawLine(0, 65, 320, 65, TFT_WHITE);
  display_obj.tft.setTextSize(1);

  display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
  display_obj.tft.setCursor(0, 16);
  display_obj.tft.print("Max ");

  display_obj.tft.setTextColor(TFT_ORANGE, TFT_BLACK);
  display_obj.tft.print("Average ");

  if (scan_mode != BT_SCAN_ANALYZER) {
    display_obj.tft.setTextColor(TFT_RED, TFT_BLACK);
    display_obj.tft.print("Channel Marker");
  }
}
